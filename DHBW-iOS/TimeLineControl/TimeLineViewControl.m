...
#define BETTWEEN_LABEL_OFFSET  (iPad ? 40 : 20)
#define LINE_WIDTH  (iPad ? 4 : 2)
#define CIRCLE_RADIUS  (iPad ? 14 : 7)
#define INITIAL_PROGRESS_CONTAINER_WIDTH  (iPad ? 50 : 20)
#define PROGRESS_VIEW_CONTAINER_LEFT  (iPad ? 65 : 51)
#define VIEW_WIDTH (iPad ? 450 : 225)
...
- (void)addProgressBasedOnLabels:(NSArray *)labels currentStatus:(int)currentStatus {
    ...
    for (UILabel *label in labels) {
        ...
        if([label.text rangeOfString:@"Fußweg"].location != NSNotFound ||
           [label.text rangeOfString:@"Stadtbus"].location != NSNotFound ||
           [label.text rangeOfString:@"Straßenbahn"].location != NSNotFound ||
           [label.text rangeOfString:@"Einsatzwagen"].location != NSNotFound ||
           [label.text rangeOfString:@"Nightliner"].location != NSNotFound ||
           [label.text rangeOfString:@"S-Bahn"].location != NSNotFound)  {
       
            if ([[UIDevice currentDevice] userInterfaceIdiom] ==UIUserInterfaceIdiomPad) {
                circle = [UIBezierPath
                          bezierPathWithRoundedRect: CGRectMake(13, yCenter-14, 50, 30)
                          cornerRadius: 24];
            } else {
                circle = [UIBezierPath
                          bezierPathWithRoundedRect: CGRectMake(5, yCenter-7, 25, 15)
                          cornerRadius: 12];
            }
            ...
            //circleLayer.contents = (__bridge id)[UIImage imageNamed:@"train"].CGImage;
        }
        else {
           circle = [UIBezierPath bezierPath];
          [self configureBezierCircle:circle withCenterY:yCenter];
            circleLayer = [self getLayerWithCircle:circle andStrokeColor:strokeColor];
