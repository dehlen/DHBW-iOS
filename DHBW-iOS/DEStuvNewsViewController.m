...
@interface DEStuvNewsViewController() 
@end

@implementation DEStuvNewsViewController
...
-(void)viewDidLoad {
    ...
    self.navigationItem.title = @"Stuv-News";
...
- (UITableViewCell *)tableView:(UITableView *)tableVie cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.titleLabel.text = [self.stuvNews[(NSUInteger) indexPath.row] title];
    cell.publishedLabel.text = [self.stuvNews[(NSUInteger) indexPath.row] published];
    cell.sneakLabel.text = [[self.stuvNews[(NSUInteger) indexPath.row] content]stringByConvertingHTMLToPlainText];

    return cell;
}

-(void)tableView:(UITableView *)tableV didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	DEStuvNewsDetailViewController *detailNewsVC = [storyboard instantiateViewControllerWithIdentifier:@"DEStuvNewsDetailViewController"];
    detailNewsVC.articles = self.stuvNews;
